...
function getMDO(loc, log) {
    ...
    const result = {
        ...
        inventory: dataObjects.readExcel(loc, "inventory")
    };
...
function generateHorizontalPlan(MDO, log) {
    ...
    var ssMatrix = dataObjects.createMatrix(dataObjects.filter(MDO.inventory, {"classification": "Safety Stock"}), "item", "period", "inventory");
    var llMatrix = dataObjects.createMatrix(dataObjects.filter(MDO.inventory, {"classification": "Level Load"}), "item", "period", "inventory");
    ...
    for(const item in MDO.items) {
        ...
        let cycleStock = null;
        let levelLoad = null;
        let strategy = MDO.items[item].strategy;
        ...
        for(let i = 0; i < periods.length; i++) {
            ...
            if(
                strategy == "VS Make-to-Stock"
            ) {
                try {
                    safetyStock = ssMatrix[item][period];
                } catch {
                    safetyStock = 0;
                }
                cycleStock = Math.round(MDO.items[item].FOQ * 0.5);
                try {
                    levelLoad = llMatrix[item][period];
                } catch {
                    levelLoad = 0;
                }
            ...
            result[item]["Level Load Plan"][period] = levelLoad;
            result[item]["Total Inv. Plan"][period] = safetyStock + cycleStock + levelLoad;
            if(i == 0){
            ...
            if(
                ...
                result[item]["Reorder Point (ROP)"][period] = Math.round(result[item]["Sales Forecast"][period] * MDO.items[item].leadTime + safetyStock + levelLoad);
            } else {
                result[item]["Reorder Point (ROP)"][period] = 0;
            }
...
/**
 * 
 * @param {Object} data - A Manufacturing Data Object that must contain a horizontal plan
 * @param {string} log - Optional logging parameter. Use "concise", "verbose", or null.
 * @returns an Aggregate Inventory data object that lists total inventory by category by period.
 * 
 */
function calculateAggInventoryPlan(data, log) {
    
    let inventoryPlans = {};
    let filter = ["Safety Stock Plan", "Cycle Stock Plan", "Level Load Plan", "Total Inv. Plan"];

    for(const item in data.horizontalPlan) {
        let crossCut = dataObjects.crossCut(data.horizontalPlan[item], filter);
        inventoryPlans[item] = computation.scalar_mult(crossCut, data.items[item].cost);
    }

    let result = computation.sum(inventoryPlans);

    switch(log) {
        case "verbose":
            console.log({
                message: "Calculate Aggregate Inventory successful.",
                result
            });
            break;
...
module.exports = {getMDO, generateHorizontalPlan, calculateAggRequirements, calculateRYG, calculateAggInventoryPlan};